...
const TopRated = () => {
  ...
  // console.log("next element", nextElement , "prev element", prevElement)
  return (
    <div className='my-16'>
          ...
          MoviesData.map((movie, index) => (
            <SwiperSlide key={index}>
              <div className='p-4 h-rate hovered border border-border bg-dry rounded-lg overflow-hidden'>
                ...
                <div className='px-4 hoveres gap-6 text-center absolute bg-black bg-opacity-70 top-0 left-0 bottom-0 right-0'>
                  <button className='w-12 h-12 flex-colo transitions hover:bg-subMain rounded-full bg-white bg-opacity-30 text-white'>
                    <FaHeart />
                  </button>
                  <Link to={`/movies/${movie.name}`} className='font-semibold text-xl truncated line-clamp-2 ' > {movie.name}
                  </Link>
                 <div className='flex flex-row gap-2 text-star'>
